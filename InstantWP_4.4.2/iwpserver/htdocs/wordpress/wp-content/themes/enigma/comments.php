<?php if ( post_password_required() ) : ?>
	...
	<div class="enigma_comment_title"><h3><i class="fa fa-comments"></i><?php echo comments_number(__('Pas de commentaires','weblizar'), __('1 Commentaire','weblizar'), '% Comments'); ?></h3></div>
	...
	<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : ?>
		<nav id="comment-nav-below">
			...
			<div class="nav-previous"><?php previous_comments_link( __( '&larr; Ancien Commentaire', 'weblizar' ) ); ?></div>
			<div class="nav-next"><?php next_comments_link( __( 'Nouveau Commentaire &rarr;', 'weblizar' ) ); ?></div>
		</nav>
		<?php endif;  ?>
	</div>		
	<?php endif; ?>
<?php if ( comments_open() ) : ?>
	...
	add_filter('wl_comment_form_default_fields','my_fields');
		...
		<textarea id="commentaire" name="commentaire" class="enigma_con_textarea_control" rows="5"></textarea></div>',		
		'logged_in_as' => '<p class="logged-in-as">' . __( "Connecté en tant que ",'weblizar' ).'<a href="'. admin_url( 'profile.php' ).'">'.$user_identity.'</a>'. '<a href="'. wp_logout_url( get_permalink() ).'" title="Se déconnecter de ce compte">'.__(", Déconnexion?",'weblizar').'</a>' . '</p>',
		'title_reply_to' => __( 'Répondre à %s','weblizar'),
		'id_submit' => 'enigma_send_button',
		'label_submit'=>__( 'Commenter','weblizar'),
